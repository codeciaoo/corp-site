...
## 取り組みの結果
- **安全情報抽出の精度向上**
  - ハイブリッド検索（ベクトル＋キーワード）で全体精度70%を達成
  - クロスエンコーダーの導入により、top3の精度が38%から68%に向上
  - さらにLLMリランキングを追加することで、最終的にtop3の精度が91%に到達
  - 作業者が最初の3件を見るだけで必要な情報を得られるようになり、情報探索時間が平均40%短縮
...
クロスエンコーダーの導入により、検索結果のtop3の精度は約38%から68%に向上しましたが、特定の複雑なケースでは依然として最適な文書が上位3件に含まれないことがありました。特に、複数の安全要素が絡み合う状況（例：「高所での溶接作業時の防火・落下防止対策」など）では、文書間の相対的な重要度の判断が難しく、クロスエンコーダーだけでは十分な精度が得られませんでした。
...
4. **処理時間の許容**
   - 本プロジェクトでは、検索結果の表示に2〜3秒程度の遅延は許容される環境だった
   - 作業計画段階での安全確認など、即時性よりも精度が重視される場面が多かった
   - LLMによる処理時間増加（約1.5秒）は、精度向上の効果と比較して十分に許容範囲内だった

LLMによるリランキングの導入により、検索結果のtop3の精度は68%から最終的に91%まで向上し、作業者が最初の3件を見るだけで必要な情報を得られるという当初の目標を達成することができました。
...
各改善段階におけるRAGシステムの性能評価には、以下の指標を使用しました：
  ...
  - **Mean Reciprocal Rank (MRR)**：各クエリに対する最初の正解文書のランク位置の逆数の平均値です。例えば、正解が1位なら1/1=1.0、2位なら1/2=0.5、3位なら1/3=0.33となります。この値が1に近いほど、正解が上位に表示されていることを意味します。
  
  - **Recall@k**：関連する全文書のうち、検索結果の上位k件に含まれる割合を示します。例えば、あるクエリに対して関連文書が10件あり、そのうち7件が検索結果の上位10件に含まれる場合、Recall@10は70%となります。
  
  - **Precision@k**：検索結果の上位k件のうち、関連文書の割合を示します。例えば、検索結果の上位3件中2件が関連文書である場合、Precision@3は67%となります。作業者が確認する文書数が限られているため、この指標は特に重要でした。

  各改善段階における評価指標の変化を以下の表にまとめました：

  | 評価指標 | ベクトル検索のみ | クロスエンコーダー導入後 | LLMリランキング導入後 | 改善率 |
  |---------|--------------|-------------------|-----------------|-------|
  | top3精度 | 38% | 68% | 91% | +53% |
  | MRR | 0.29 | 0.52 | 0.83 | +0.54 |
  | Recall@10 | 72% | 85% | 94% | +22% |
  | Precision@3 | 42% | 73% | 89% | +47% |

  これらの指標の大幅な向上により、以下のユーザー体験の改善が実現しました：
  
  - 作業者の情報探索時間
  - 安全確認の実施率（作業前の安全確認をスキップするケースが減少）
