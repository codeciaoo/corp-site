...
- **top3精度の改善効果：**
  - ベクトル検索のみ：top3に最適な回答が含まれる確率 38%
  - クロスエンコーダー導入後：top3に最適な回答が含まれる確率 68%
  - LLMリランキング導入後：top3に最適な回答が含まれる確率 91%
  - 作業者の情報探索時間：平均40%短縮（1回あたり約45秒→27秒）
  - 安全確認の実施率：15%向上（作業前の安全確認をスキップするケースが減少）
  - 検索結果表示までの時間：約2.5秒（許容範囲内）
